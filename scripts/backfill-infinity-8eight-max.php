...
/**
 * Backfill: agregar space ofXathJUIaDh (8EIGHT MAX.) a TODOS los suscriptores
 * activos de Infinity (product_ids 6454766/7065704/6952229).
 *
 * El space ya está en la config del product_key 'infinity', así que los grants
 * NUEVOS ya lo incluyen. Este script lo asigna a los miembros que ya existían
 * antes de agregarlo. Idempotente a nivel Bettermode (re-grant no rompe).
 *
 * Throttle 1s entre calls. Resume-safe (relee el .log y salta lo ya hecho).
 * Uso:  php scripts/backfill-infinity-8eight-max.php [--dry-run]
 */
...
const SPACE_ID     = 'ofXathJUIaDh';                 // 8EIGHT MAX.
const SLEEP_SEC    = 1;
const REPORT_EVERY = 25;
$LOG_PATH = $root . '/scripts/.backfill-infinity-8eight-max.log';
...
$emails = $pdo->query(
    "SELECT DISTINCT LOWER(subscriber_email) AS email
       FROM subscriptions
      WHERE product_id IN ('6454766','7065704','6952229')
...
echo "Backfill space 8EIGHT MAX. (ofXathJUIaDh) a Infinity activos\n";
echo "============================================================\n";
echo "Space    : ofXathJUIaDh (8EIGHT MAX.)\n";
