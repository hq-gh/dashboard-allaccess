...
final class UsersRepo
{
    ...
    public function listAll(): array
    {
        return Database::get()->query(
            "SELECT id, name, email, role, created_at, updated_at, last_login_at
               FROM public.users ORDER BY role DESC, name ASC"
    ...
    {
        $st = Database::get()->prepare("SELECT 1 FROM public.users WHERE LOWER(email) = LOWER(:e) LIMIT 1");
        $st->execute([':e' => $email]);
    ...
    {
        $st = Database::get()->prepare("UPDATE public.users SET name = :n, role = :r WHERE id = :id");
        $st->execute([':n' => $name, ':r' => $role, ':id' => $id]);
    }

    public function updatePassword(int $id, string $passwordHash): void
    {
        $st = Database::get()->prepare("UPDATE public.users SET password_hash = :ph WHERE id = :id");
        $st->execute([':ph' => $passwordHash, ':id' => $id]);
    }

    public function delete(int $id): void
    {
        $st = Database::get()->prepare("DELETE FROM public.users WHERE id = :id");
        $st->execute([':id' => $id]);
    }

    public function countAdmins(): int
    {
        return (int) Database::get()->query("SELECT COUNT(*) FROM public.users WHERE role = 'administrador'")->fetchColumn();
    }
}
