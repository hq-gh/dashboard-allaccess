<?php
// ========================================
// DASHBOARD PRINCIPAL Infinity VIP → INFINITY
// ========================================
...
<body>
        ...
        <div class="logo-container">
            <div class="logo">
                <img src="https://github.com/hq-gh/dashboard-allaccess/blob/main/5T4D10%20logo.png?raw=true"
                     alt="5T4D10 Logo" class="logo-img"
                     onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                <div style="display:none; color: #FF6687; font-size: 24px; font-weight: 700;">5T4D10</div>
            </div>
        </div>
        <div class="user-info">
            <span>Sesión: <?= htmlspecialchars($username, ENT_QUOTES, 'UTF-8') ?></span>
            <a href="logout.php" class="logout-btn"
               onclick="return confirm('¿Estás seguro que deseas cerrar sesión?');">Cerrar Sesión</a>
        </div>
    </header>
    ...
    <main class="main-container">
        ...
        <div class="content-section">
            ...
            <div id="exportSection" class="export-section">
                ...
                <a href="export-excel.php" class="export-btn">📥 DESCARGAR EXCEL</a>
            </div>
    ...
    <footer class="footer">
        Dashboard v3.1.0 | 5T4D10 CTO Team | Mérida, Yucatán<br>
        Powered by Railway.
    </footer>

    <script>
        // ---------- Helpers ----------
        // Escape HTML para evitar XSS al insertar datos en innerHTML
        ...
        async function syncData() {
            ...
            try {
                ...
                // Si la sesión expiró durante el dashboard
                ...
                // Estadísticas (acepta llave nueva o vieja para compatibilidad)
                if (data.stats) {
                    const totalVip = data.stats.total_infinity_vip ?? data.stats.total_all_access ?? 0;
                    const pecadores = data.stats.pecadores ?? 0;
                    const noPecadores = data.stats.no_pecadores ?? 0;
                    const conversionRate = data.stats.conversion_rate;

                    let conversionCard = '';
                    if (conversionRate !== undefined && conversionRate !== null) {
                        conversionCard = `
                            <div class="stat-card">
                                <div class="stat-number">${escapeHtml(conversionRate)}%</div>
                                <div class="stat-label">Tasa de Conversión</div>
                                <div class="stat-sublabel">No Pecadores / Total Infinity VIP</div>
                            </div>`;
                    }

                    statsContent.innerHTML = `
                        <div class="stat-card">
                            <div class="stat-number">${escapeHtml(pecadores)}</div>
                            <div class="stat-label">Pecadores</div>
                            <div class="stat-sublabel">Tienen Infinity VIP y no tienen INFINITY</div>
                        </div>
                        <div class="stat-card">
                            <div class="stat-number">${escapeHtml(totalVip)}</div>
                            <div class="stat-label">Infinity VIP</div>
                            <div class="stat-sublabel">Total de usuarios con Infinity VIP activo</div>
                        </div>
                        <div class="stat-card">
                            <div class="stat-number">${escapeHtml(noPecadores)}</div>
                ...
                if (data.data && data.data.length > 0) {
                    ...
                    html += '<thead><tr>'
                         ...
                         + '<th>Código Transacción</th><th>Fecha/Hora</th><th>Plan</th><th>Precio</th>'
                         + '</tr></thead><tbody>';
                    ...
                    data.data.forEach(user => {
                        html += '<tr>'
                            ...
                            + '<td class="country">' + escapeHtml(user.country) + '</td>'
                            + '<td class="phone">' + escapeHtml(user.phone) + '</td>'
                            + '<td class="transaction">' + escapeHtml(user.codigo_transaccion) + '</td>'
                            + '<td class="fecha">' + escapeHtml(formatDate(user.fecha_hora)) + '</td>'
                            + '<td class="plan">' + escapeHtml(user.plan) + '</td>'
                            + '<td class="precio">' + escapeHtml(formatPrice(user.precio, user.moneda)) + '</td>'
                            + '</tr>';
                    });
                    ...
                    debugDiv.textContent = '✅ ' + data.data.length + ' usuarios cargados exitosamente';
                } else {
                    contentDiv.innerHTML = '<div class="loading">No hay datos para mostrar</div>';
                    debugDiv.textContent = '⚠️ No hay usuarios para mostrar';
                }
